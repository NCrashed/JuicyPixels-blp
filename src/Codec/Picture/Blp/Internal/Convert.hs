...
toBlpCMYK8 :: Image PixelRGBA8 -> Image PixelRGBA8 
toBlpCMYK8 = pixelMap convert 
  where
  clampWord8 = fromIntegral . max 0 . min 255 . (`div` 255)
  convert (PixelRGBA8 r g b a) = PixelRGBA8 (clampWord8 c) (clampWord8 m) (clampWord8 y) a
    where
    ik = fromIntegral a :: Double
    c, m, y :: Int
    c = round $ fromIntegral b / ik
    m = round $ fromIntegral g / ik
    y = round $ fromIntegral r / ik
